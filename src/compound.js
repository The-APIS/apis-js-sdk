...
export default class Compound {
  async init(apiKey) {
    if (!apiKey) return console.error('Visit https://app.theapis.io to create a [free] account and API token.')
  ...
  async enterMarket() {
    try {
      const errorCode = await this.comptroller.instance.methods
        .enterMarkets(this.cTokenAddresses)
  ...
  async getInvestBalance(tokenName) {
    return tokenName === 'ETH'
      ? await this.cETH.instance.methods.balanceOfUnderlying(this.accounts[0]).call()
      : this.cERC20
          .find((ERC20) => ERC20.name === tokenName)
          .instance.methods.balanceOfUnderlying(this.accounts[0])
          .call()
  }
  ...
  async getAPY(tokenName) {
    ...
    var rate = await (tokenName === 'ETH'
      ? this.cETH.instance.methods.supplyRatePerBlock().call()
      : this.cERC20
          .find((ERC20) => ERC20.name === tokenName)
          .instance.methods.supplyRatePerBlock()
          .call())
