...
export default class Earn {
  async init() {
    ...
    console.log('fetching network successful: ')
  ...
  async invest(tokenName, amount) {
    ...
    await ytoken.instance.methods.deposit(amount).send({ from: this.accounts[0] })
  }
  ...
  async withdraw(tokenName, amount) {
    ...
    await ytoken.instance.methods.withdraw(amount).send({ from: this.accounts[0] })
  }
  ...
  async getTokenBalance(tokenName) {
    ...
    console.log('current conversion rate of ' + ytoken.name + ' is')
    console.log(price)
